...
The system SHALL allow users to rename an existing scraping session. Renaming MUST update the `sessionId` for all data items belonging to that session in the `scrapedData` storage.

#### Scenario: Rename Bali session to Lombok
- **WHEN** user inputs "Lombok" as the new name for the "Bali" session and confirms
- **THEN** all 50 items previously tagged as "Bali" now have `sessionId: "Lombok"` and the UI updates to show the new session name.
